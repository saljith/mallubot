...
# Enter your Telegram ID here to use /ban, /unban and /broadcast
...
def start(bot, update):
    ...
    reply_markup1 = telegram.ReplyKeyboardMarkup(
        keyboard1, resize_keyboard=True)
    bot.send_chat_action(chat_id=update.message.chat_id,
                         action=telegram.ChatAction.TYPING)

    if user_id in db.get_banned_users():
        bot.send_message(user_id, "{} You have been banned from using this bot!".format(
    ...
    if (user_id not in searching_users) and (user_already_chatting(user_id) == -1):
        ...
        if len(searching_users) > 0:
            ...
            bot.send_message(chat_id=update.message.chat_id,
                             text="❇️ Searching for strangers! ❇️", reply_markup=reply_markup)

    elif user_id in searching_users:
        bot.send_message(user_id, "{} {}".format(
            BOT_SENDS, "You are already searching. Please wait!"), parse_mode="Markdown")
...
def stop(bot, update):
    ...
    keyboard = [[InlineKeyboardButton("💥New Chat💥", callback_data='1'), InlineKeyboardButton(
        "🍴Settings🍴", callback_data='2')], [InlineKeyboardButton("🚀Help & Info🚀", callback_data='3')]]
    bot.send_chat_action(chat_id=update.message.chat_id,
                         action=telegram.ChatAction.TYPING)
    reply_markup = telegram.ReplyKeyboardMarkup(keyboard, resize_keyboard=True)
    ...
    if (user_id in searching_users) or (user_already_chatting(user_id) >= 0):
        ...
        if user_id in searching_users:
            ...
            bot.send_message(chat_id=update.message.chat_id,
                             text="📛 At this moment you are not searching 📛", reply_markup=reply_markup)
            ...
            bot.send_message(partner_id, text="🎈 Your partner left the chat 🎈",
                             reply_markup=reply_markup, parse_mode="Markdown")
            bot.send_message(
                user_id, text="🎈 You left the chat! 🎈", reply_markup=reply_markup)
...
def broadcast(bot, update, args):
    ...
    for user_id in users:
        bot.send_message(user_id, "{} {}".format(
            BOT_BROADCAST, text), parse_mode="Markdown")
      ...
      bot.send_message(user_id, "{} {}".format(
       ...
       if partner_id != -1:
           if photo is not None:
               ...
               bot.send_document(
                   partner_id, document=document.file_id, caption=caption)
               ...
               bot.send_message(
                   partner_id, text="{} {}".format(STRANGER_SENDS, text))
...
def start_tcb(bot, update, args):
    ...
    bot.sendMessage(
        chat_id=update.message.chat_id, text="Hello {}, I'm HMSU Radio Bot.".format(
            user_data.username)
    )
    ...
    keyboard = [[InlineKeyboardButton("START", callback_data='1'), InlineKeyboardButton(
        "Settings", callback_data='2')], [InlineKeyboardButton("Help & Info", callback_data='3')]]
...
handlers.append(RegexHandler('💥New Chat💥', start))

handlers.append(RegexHandler('❌Left the Chat❌', stop))
handlers.append(RegexHandler('❌Stop Searching❌', stop))
