...
def start(bot, update):
    ...
    if (user_id not in searching_users) and (user_already_chatting(user_id) == -1):
        ...
        if len(searching_users) > 0:
            ...
            valeues=telegram.Bot.link
            bot.send_message(user_id, "{} {}".format(BOT_SENDS, "Searching for strangers!"+valeues), parse_mode="Markdown")
...
handlers.append(CommandHandler('start',start_tcb, pass_args=True))
...
handlers.append(RegexHandler('Settings', start))
